...
def create_startup_script():
    ...
    go_program_path = script_dir / "dist" / "sg-whitelist-automation.exe"
    ...
    if not go_program_path.exists():
        print(f"Error: Go executable not found at {go_program_path}. Please compile the Go program first.")
        sys.exit(1)
    ...
    try:
        ...
        print(f"Batch file created at {batch_file_path}. The program will run automatically when you log in.")
        print("Note: This method creates a batch file to start the program, as direct .lnk creation with specific properties requires external libraries.")
